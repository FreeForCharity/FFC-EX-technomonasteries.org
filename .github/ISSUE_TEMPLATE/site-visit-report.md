---
name: Site Visit Report
about: Document a visit to a candidate property
title: '[SITE VISIT] '
labels: 'site-development, research'
assignees: ''
---

## Visit Details

**Property:**
**Date of Visit:**
**Visitors / Team Members:**

## Condition Assessment

### Structures
<!-- Describe the condition of any existing buildings or structures -->


### Utilities
<!-- Power, water, sewage — what exists, what's needed -->


### Land
<!-- Terrain, soil, vegetation, drainage, access roads -->


### Surrounding Area
<!-- Neighborhood, nearby properties, local amenities visited -->


## Key Observations

<!-- What stood out? Surprises? Things not visible in listings/photos? -->

-
-
-

## Photos / Video

<!-- Link to photos, video, or attach to this issue -->

-

## Local Contacts Made

<!-- Did you meet neighbors, officials, agents? -->

| Name | Role | Contact | Notes |
|------|------|---------|-------|
| | | | |

## Internet Test (If Possible)

**Speed test results:**
**Cell signal strength:**
**Starlink viability notes:**

## Recommendation

- [ ] **Proceed** — property meets our needs, recommend next steps
- [ ] **Needs More Info** — follow-up visit or research required
- [ ] **Pass** — property does not meet our needs

**Reasoning:**


## Next Steps

- [ ]
- [ ]
